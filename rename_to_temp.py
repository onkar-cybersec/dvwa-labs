#!/usr/bin/env python3
"""
Rename image files to temporary names and update Markdown references.
...
def temp_name(name: str) -> str:
    path = Path(name)
    if path.stem.endswith(TEMP_MARKER):
        return name
    return f"{path.stem}{TEMP_MARKER}{path.suffix}"
...
def find_image_files(root: Path) -> list[Path]:
    return sorted(
        ...
        and not path.stem.endswith(TEMP_MARKER)
    )
...
def build_rename_map(root: Path) -> dict[str, str]:
    rename_map: dict[str, str] = {}

    for image in find_image_files(root):
        target = image.with_name(temp_name(image.name))
        if target.exists():
            print(f"Skipped existing temp target: {display_path(target, root)}")
            continue

        rename_map[display_path(image, root)] = display_path(target, root)

    return rename_map


def update_markdown_references(root: Path, rename_map: dict[str, str]) -> set[str]:
    updated_files: set[str] = set()
    lower_map = {old.lower(): new for old, new in rename_map.items()}
...
def rename_images(root: Path, rename_map: dict[str, str]) -> int:
    count = 0
    for old_relative, new_relative in sorted(rename_map.items()):
...
def main() -> int:
    root = repo_root()
    rename_map = build_rename_map(root)

    if not rename_map:
        print("Images renamed: 0")
        print("Done.")
        return 0

    updated_files = update_markdown_references(root, rename_map)
    renamed_count = rename_images(root, rename_map)
